...
- The frontend is wired to this — see the Frontend section above. `backend/shared-types` mirrors `frontend/src/types.ts` and is the contract both sides are expected to stay in sync with. Known gap: there's no UI yet for assigning specific cloud connections to an invited Viewer (`accountConnectionIds`), so newly-invited Viewers see zero VMs until an Admin assigns some other way (e.g. a direct `PATCH /api/users/:id` call).
