...
- Data: no more mock data file — `src/api/client.ts` holds typed fetch wrappers for every BFF route, and `src/state/useCirrusApp.ts` is fetch-driven (bootstraps the session via `GET /auth/me`, then loads `providers`/`vms`/`connections`/`users` from the backend; `providers` now carries `fieldDefs`/`checklist`/`failureMessage` per-provider, replacing the old static `FIELD_DEFS`/`CHECKLIST`/`FAILURE_MSG` maps from PRD §7.3). Dev server proxies `/api` and `/auth` to the BFF (`vite.config.ts`'s `server.proxy`) so the session cookie stays same-origin — always fetch relative paths, never an absolute `localhost:8080` URL.
