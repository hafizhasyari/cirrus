...
Cirrus is an internal, read-only dashboard that aggregates VM inventory across the company's own accounts on 5 cloud providers — **AWS, GCP, Alibaba Cloud, OCI, and Biznet Gio Cloud**. It is not a multi-tenant SaaS product.

Current MVP scope: inventory listing with filter/search, plus RBAC/SSO (2 roles: `Admin`, `Viewer`). Cost/billing was deliberately cut from the MVP — see `PRD.md` §10 Roadmap.
