...
> All data shown below is fictional/demo data for illustration only.

| | |
|---|---|
| ![Sign-in screen](docs/screenshots/login.png) | ![Inventory across 5 providers](docs/screenshots/inventory.png) |
| Sign in with your company Microsoft account | Every VM, across every provider, in one table |
| ![VM detail drawer](docs/screenshots/vm-detail.png) | ![Cloud Connections](docs/screenshots/connections.png) |
| Full spec + per-disk storage for a single VM | Admins manage which cloud accounts Cirrus pulls from |

![Users & Roles](docs/screenshots/users.png)

Admins invite users, assign roles, and scope Viewers to specific cloud accounts.
