...
func main() {
	...
	mux.Handle("GET /instances", metrics.Wrap(collectorkit.RequireInternalSecret(internalSecret, collectorkit.RateLimit(limiter, collectorkit.WithTimeout(http.HandlerFunc(handleInstances), 20*time.Second))), "instances"))
