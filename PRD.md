...
|---|---|---|---|
| AWS | EC2 `DescribeInstances` + **Lightsail `GetInstances`** | **Static IAM User Access Key** (per-akun, read-only — `ec2:Describe*`, opsional `lightsail:*` kalau akunnya juga punya instance Lightsail — lihat §7.3) | Paling matang API-nya. Lightsail jalan di collector yang sama, bukan provider baru — kredensialnya sama dengan EC2 (lihat §11 poin 6) |
...
| OCI | API Signing Key (statis, per-akun — tetap pola yg benar krn tiap tenancy OCI terpisah, bukan workload di dalam OCI) | **Tenancy OCID**, **User OCID**, **Fingerprint**, **Private Key** (upload/paste), **Region** (cuma titik awal/bootstrap — beda dgn AWS/Alibaba yg bisa hardcode 1 region global karena OCI gak punya region universal lintas realm OC1/OC2/dst, jadi Admin tetap wajib isi 1 region valid apapun di realm tenancy tsb, gak perlu region tempat VM-nya berada — Collector otomatis discover semua region yg di-subscribe tenancy via `identity.list_region_subscriptions()` dan fetch dari semuanya), opsional **Passphrase** | `oci.config.validate_config()` (cek format) → `identity.list_region_subscriptions()` (tenancy-scoped, cek signing valid + hitung region yg di-subscribe tenancy ini, bisa dari region manapun di realm yg sama — gak wajib home region) → `compute.list_instances()` limit=1 (konfirmasi read-only policy benar-benar attached) | Private Key + field lain di **Vault KV v2**, per-koneksi (tiap tenancy OCI punya masing-masing). Disarankan bikin IAM user + policy read-only khusus di tenancy tsb (`allow group ReadOnlyGroup to inspect/read instance-family in tenancy`), bukan pakai akun tenancy-admin. |
...
6. ~~AWS Lightsail di luar scope MVP~~ — **Resolved**: AWS Collector (§7.2) sekarang juga manggil Lightsail API (`lightsail:GetRegions`/`GetInstances`), jalan di collector yang sama dengan EC2 — bukan provider baru, karena kredensialnya memang akun AWS yang sama. Kalau IAM user yang didaftarkan cuma punya `AmazonEC2ReadOnlyAccess` (policy default di setup guide), Lightsail-nya di-skip secara graceful dan EC2 tetap jalan seperti biasa — supaya muncul, tinggal tambah custom policy kecil (`lightsail:GetRegions` + `lightsail:GetInstances`) di IAM user yang sama, tanpa perlu bikin koneksi baru. Catatan: AWS ternyata tidak punya managed policy read-only khusus Lightsail — satu-satunya policy Lightsail-specific dari AWS (`LightsailExportAccess`) cuma untuk service-linked role dan tidak bisa di-attach ke IAM user biasa, jadi custom policy memang satu-satunya opsi yang sesuai prinsip least-privilege di proyek ini (alternatifnya, policy job-function `ReadOnlyAccess` bawaan AWS juga cover Lightsail, tapi itu read-only ke *semua* service AWS, jauh lebih luas dari yang dibutuhkan). VM Lightsail ditandai dengan label "Lightsail" di UI biar kebeda dari EC2. Catatan: Alibaba Cloud sebenarnya punya produk setara (Simple Application Server/SWAS, API `swas-open` yang terpisah dari ECS) yang berpotensi punya gap serupa — sudah dicek, **tidak relevan**: perusahaan tidak pakai SWAS, cuma ECS, jadi tidak ada development tambahan yang diperlukan di Alibaba Collector.
