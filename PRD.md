...
- **Platform-level "hub" credential**: untuk Alibaba Cloud, Cirrus butuh 1 kredensial milik Cirrus sendiri (bukan per-akun) yang dipakai buat `AssumeRole` ke RAM Role yang dibuat di tiap akun Alibaba yang didaftarkan — ini disimpan sekali di Vault (dipakai oleh Alibaba Collector), beda dari kredensial per-koneksi yang cuma berupa Role ARN + Region ID (bukan secret, aman disimpan di Postgres). Detail lengkap di §7.3.
- **AWS tidak pakai model hub/AssumeRole** — tiap akun AWS yang didaftarkan punya IAM User + Access Key sendiri (Access Key ID di Postgres, Secret Access Key di Vault KV v2 per-koneksi), sama seperti pola OCI/Biznet Gio. Setup jadi lebih sederhana (tidak perlu trust policy lintas akun), dengan trade-off memakai long-lived static key dibanding role assumption/temporary credential yang sebetulnya direkomendasikan AWS sendiri — keputusan sadar demi kesederhanaan operasional. Detail lengkap di §7.3.
...
- **HashiCorp Vault (KV v2)**: static secrets engine untuk field kredensial yang genuinely secret (lihat §6.2/§7.3 — hari ini OCI `Private Key`/`Passphrase`, Biznet Gio `x-token`, dan AWS `Secret Access Key`; field GCP/Alibaba semuanya identifier non-secret jadi cukup di Postgres, begitu juga AWS `Access Key ID`). RBAC adalah satu-satunya client Vault di seluruh sistem.

### 7.2 Integrasi per Provider

| Provider | Instance Data Source | Auth Method | Notes |
|---|---|---|---|
| AWS | EC2 `DescribeInstances` | **Static IAM User Access Key** (per-akun, read-only — `ec2:Describe*`) — lihat §7.3 | Paling matang API-nya |
...
Detail field form "Tambah Koneksi Akun Cloud" (§5) dan mekanisme validasinya, hasil riset context7 + websearch (per keputusan: Alibaba pakai cross-account role, GCP pakai Workload Identity Federation — bukan static key, sesuai current best practice tiap provider; AWS pakai static per-akun IAM Access Key, sama seperti OCI & Biznet Gio — pilihan sadar demi kesederhanaan setup per-akun, meski AWS sendiri merekomendasikan role assumption/temporary credential dibanding long-lived key):
...
5. **Setup cross-account role/WIF per akun cloud** — dgn pola Alibaba RAM Role dan GCP Workload Identity Federation (§7.3), tim yang pegang akun Alibaba/GCP perlu bikin role/trust policy sendiri per akun (bukan cuma copy-paste 1 static key) — ini lebih secure tapi setup-nya lebih banyak langkah dibanding AccessKey/Secret biasa. AWS sengaja dikecualikan dari pola ini (pakai static per-akun IAM Access Key, sama seperti OCI/Biznet Gio) demi kesederhanaan setup, dengan trade-off keamanan yang sudah didiskusikan di §6.2. GCP WIF khususnya butuh Cirrus punya OIDC issuer/JWKS endpoint sendiri (bagian dari Auth Service) — perlu dipastikan ini bisa dibangun sebelum development GCP Collector dimulai.
