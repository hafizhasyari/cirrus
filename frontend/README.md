...
React + TypeScript + Vite SPA for the Cirrus Cloud VM Inventory Dashboard. Ported from the `Cirrus.dc.html` design mockup; fully wired to the real backend via `src/api/client.ts` — no mock data. See [`DEVELOPMENT.md`](../DEVELOPMENT.md) for full-stack setup (this app talks to `bff`, `auth`, `rbac`, and `aggregator`, run via the root `docker-compose.yml`).
